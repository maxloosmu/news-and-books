				...
				<table class="form-table" role="presentation">
					...
					<?php if ( get_option( 'show_avatars' ) ) : ?>
						<tr class="user-profile-picture">
							...
							<td>
							<?php echo "Testing if this file is being executed."; ?>
							<?php 
							$avatar_html = get_avatar( $user_id, 96 ); // Get the Gravatar HTML
							$alt_text = esc_attr( sprintf( __( 'Default profile picture for user ID %d' ), $user_id ) ); // Create alt text
							// Replace or add the alt attribute in the avatar HTML
							$avatar_with_alt = preg_replace('/alt=[\'"].*?[\'"]/', 'alt="' . $alt_text . '"', $avatar_html);
							if ( ! preg_match('/alt=/', $avatar_with_alt) ) {
								$avatar_with_alt = str_replace('<img', '<img alt="' . $alt_text . '"', $avatar_with_alt);
							}
							echo $avatar_with_alt; 
							?>
							<?php echo "Testing TWICE if this file is being executed."; ?>
								<p class="description">
									<?php
									if ( IS_PROFILE_PAGE ) {
										$description = sprintf(
